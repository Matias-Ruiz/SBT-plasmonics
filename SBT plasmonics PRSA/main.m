...
%% Shape function f
f_s = sqrt(1-z.^2); % f for a spheroid
f = (f_s).*(3+z)/3; % f for a asymmetric shape
...
% eigenvalues of spheroid v/s eigenvalues of asymmetric shape
figure
plot(sort(lam_spher,'descend'),'*')
hold on
plot(lam_kk,'o')
axis([0,20,0,200])
legend('spheroid', 'asymmetric shape','interpreter', 'latex'); legend('boxoff');
title('eigenvalues')
...
% SBT approximation of the polarizability for the asymmetric shape
for i = 1:size(m,1)
...
% SBT approximation of the polarizability for a prolate spheroid
...
% SBT approximation of the averaged absorption cross section per unit of volume: spheriod v/s asymmetric shape
figure; 
plot(wavelength , 2*pi/3./wavelength.*imag(pol_ex)/(4/3*pi*h^2), 'k', 'LineWidth',1); hold on; plot(wavelength , 2*pi/3./wavelength.*imag(polarizability)/trapz(h^2*f.^2*pi*dz),'--r','LineWidth',1); 
legend('spheroid', 'asymmetric shape','interpreter', 'latex'); legend('boxoff'); xlabel('wavelength $(nm)$', 'interpreter', 'latex'); ylabel('averaged absorption cross section / $V$ ($nm^{-1}$)','interpreter', 'latex');
...
% averaged absorption cross section per unit of volume of a spheriod: SBT approximation v/s exact values
figure;  
...
legend('exact', 'asymptotic','interpreter', 'latex'); legend('boxoff'); xlabel('wavelength $(nm)$', 'interpreter', 'latex'); ylabel('averaged absorption cross section / $V$ ($nm^{-1}$)','interpreter', 'latex');
grid off; set(gca, 'YScale', 'log'); 
